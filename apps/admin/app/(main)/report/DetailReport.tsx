...
export default function AdminReportDetailPopup({
  ...
  const displayViolation = (v: string) =>
    v === "copyright" ? "Copyright Violation" : "Comment Violation";

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div
        className="relative w-[680px] rounded-2xl bg-white/80 backdrop-blur-xl shadow-2xl p-8 border border-white/40"
      >
        {/* Header */}
        ...
        <div className="space-y-4 text-[15px] text-neutral-700">
          <div className="grid grid-cols-4 gap-y-2">
            ...
            <p className="col-span-3">{displayViolation(user.violate)}</p>

            <p className="font-medium text-neutral-900">Reported By</p>
            <p className="col-span-3">{user.annunciator}</p>

            <p className="font-medium text-neutral-900">Report Date</p>
            <p className="col-span-3">
              {user.reportDate.toLocaleDateString("en-US")}
            </p>
          </div>

          {/* Content */}
          <div className="mt-4">
            <p className="font-medium text-neutral-900 pb-1">
              Detailed Description
            </p>
            <div
              className="p-4 rounded-xl bg-neutral-100 text-neutral-800 text-sm leading-relaxed shadow-inner"
            >
              {user.reportContent}
            </div>
          </div>
        ...
        <div className="flex justify-center gap-6 mt-8">
          <Button
            className="rounded-full px-8 py-2 text-lg bg-orange-500 hover:bg-orange-600 transition text-white shadow-md"
          >
            Issue Warning
          </Button>

          <Button
            className="rounded-full px-8 py-2 text-lg bg-red-600 hover:bg-red-700 transition text-white shadow-md"
          >
            Block Account
          </Button>
        </div>
